...
```
WINEESYNC=1 WINEFSYNC=1 DXVK_ASYNC=1 PROTON_NVIDIA_LIBS_NO_32BIT=1 __GL_SHADER_DISK_CACHE_SKIP_CLEANUP=1 MANGOHUD=1 MANGOHUD_CONFIG=position=top-right,fps=0,cpu_stats=0,gpu_stats=0 game-performance %command%
```

Append game-specific flags after `%command%` (e.g. `-dx11`).

> `MANGOHUD_CONFIG=position=top-right,fps=0,cpu_stats=0,gpu_stats=0` — shows only the frametime graph. Frametime is more useful than FPS: a stable 60fps with spiking frametimes means stutters; frametime exposes that instantly.
...
**Launch options**:
```
WINEESYNC=1 WINEFSYNC=1 DXVK_ASYNC=1 PROTON_NVIDIA_LIBS_NO_32BIT=1 __GL_SHADER_DISK_CACHE_SKIP_CLEANUP=1 MANGOHUD=1 MANGOHUD_CONFIG=position=top-right,fps=0,cpu_stats=0,gpu_stats=0 game-performance %command% -dx11
```
...
**Launch options**:
```
WINEESYNC=1 WINEFSYNC=1 PROTON_NVIDIA_LIBS_NO_32BIT=1 __GL_SHADER_DISK_CACHE_SKIP_CLEANUP=1 MANGOHUD=1 MANGOHUD_CONFIG=position=top-right,fps=0,cpu_stats=0,gpu_stats=0 game-performance %command%
```
